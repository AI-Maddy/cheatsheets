**cheat sheet** for the **Linux V4L2** (Video4Linux2) framework — focused on the most common patterns used in 2024–2025 (kernel 6.x era).

### Quick Command-line Exploration Tools
...
### Typical User-Space Capture Sequence (most common today)
...
ioctl(fd, VIDIOC_QUERYCAP, &cap);               // → cap.capabilities & cap.device_caps
...
ioctl(fd, VIDIOC_S_FMT, &fmt);                  // driver may change values → read back

// 3. Request MMAP buffers (most common & fastest method)
struct v4l2_requestbuffers req = {0};
req.count  = 4;                                 // usually 3–8
...
// 7. Main loop: dequeue → process → re-queue
while (running) {
...
### Most Important V4L2 ioctls (Quick Reference Table)

Category               | ioctl                          | Main arg struct                  | Purpose / Notes
-----------------------|--------------------------------|----------------------------------|-----------------------
Capabilities           | `VIDIOC_QUERYCAP`              | `v4l2_capability`                | Must call first — read `capabilities`, `device_caps`, `card`, `driver`
Format                 | `VIDIOC_ENUM_FMT`              | `v4l2_fmtdesc`                   | List supported pixelformats (index++)
Format                 | `VIDIOC_G_FMT` / `VIDIOC_S_FMT`| `v4l2_format`                    | Get / set width/height/pixelformat/field/…
Format                 | `VIDIOC_TRY_FMT`               | `v4l2_format`                    | Dry-run (does not commit)
Buffers                | `VIDIOC_REQBUFS`               | `v4l2_requestbuffers`            | count=0 → free, count>0 → alloc
...
Controls (ext)         | `VIDIOC_(G|S|QUERY)_EXT_CTRLS` | `v4l2_ext_controls`              | Modern way — arrays, 64-bit, volatile/…
...
Events                 | `VIDIOC_SUBSCRIBE_EVENT`       | `v4l2_event_subscription`        | SOURCE_CHANGE, CTRL, FRAME_SYNC, …
Events                 | `VIDIOC_DQEVENT`               | `v4l2_event`                     | Read events (non-blocking or with poll)

### Most Common Pixel Formats (fourcc)
...
`YUYV` / `YUY2` | → most universal fallback
`RGB3` / `RGBP` | 24-bit RGB                         | uncompressed preview
`XR24` / `RGBA` | 32-bit RGBA                        | GUI / OpenGL

### Quick v4l2-ctl Examples

```bash
# Show everything about device
...
# Show media topology (very useful for complex SoC cameras)
media-ctl -p -d /dev/media0
```

### Kernel Side — Quick Driver Skeleton Reminders
...
Good luck with your V4L2 project!